# THis is main file to run the linear regression code with different options
# We can run the code with different options using ArgumentParser from argparse library in Python as follows:
# pre-processing: 0 for no processing, 1 for min/max scaling and 2 for standrizing
# choice: 0 for linear verification, 1 for training with all features, 2 for training with the best feature, 3 for normal equations, 4 for sikit)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--preprocessing', type=int, default=1,       # for preprocessing selection 
                        help='0 for no processing, '
                             '1 for min/max scaling and '
                             '2 for standrizing')

    parser.add_argument('--choice', type=int, default=2,
                        help='0 for linear verification, '            # perfect line 45 degree
                             '1 for training with all features, '     #  all features training 
                             '2 for training with the best feature, ' # best feature training for each feature comparison with target
                             '3 for normal equations, '               # normal equations solution  # (X.T * X)^(-1) * X.T * y
                             '4 for sikit)')                          # scikit learn solution
